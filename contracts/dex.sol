...
contract Dex is Wallet {
    ...
    function createMarketOrder(Side side, bytes32 ticker, uint amount) public {
        if (side == Side.SELL){
            require(balances[msg.sender][ticker] > amount, "Not sufficient token balance");
        }
        ...
        for (uint256 i = 0; i < orders.length && totalFilled < amount; i++) {
            ...
            if (availableToFill > leftToFill) {
                filled = leftToFill; //Exitem fundos suficientes no orderbook para preencher o tamanho da ordem
            }

            else {
                filled = availableToFill; //Não existe fundos suficientes para preencher o tamanho da ordem em order[i]
            }
